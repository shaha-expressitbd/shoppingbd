...
export default function CheckoutPage() {
  ...
  // Redirect if no items logic removed (Only tracking remains)
  useEffect(() => {
    // Empty cart notification logic removed as requested
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    console.log("=== CHECKOUT ORDER REQUEST PAYLOAD ===", orderPayload);
    ...
    try {
      ...
      if (response.success) {
        ...
        // Store Customer Data for GTM EMQ
        storeCustomerData({
          name: formData.name,
        ...
        // Track order placement for ALL orders (COD + Online)
        trackOrderPlace({
        ...
        if (formData.paymentMethod === "cashOnDelivery") {
          // Track COD-specific event
          trackCODPlace(backendOrderId || orderId, total, displayItems);
