...
function getCategorySelectionState(
  ...
  // If this category itself is selected, or any of its descendants are selected
  const isDirectlySelected = selectedCats.includes(cat._id);
  if (isDirectlySelected) return "all";

  // Check if any descendants are selected
  const hasSelectedDescendants = getAllDescendantIds(cat).some(id => selectedCats.includes(id) && id !== cat._id);
  if (hasSelectedDescendants) return "partial";
  ...
  const toggleCategoryWithChildren = useCallback(
    (cat: Category) => {
      setSelectedCats((prev) => {
        const isSelected = prev.includes(cat._id);
        return isSelected
        ...
        return (
          ...
          >
            <div className="flex items-center group">
              ...
              <label className="flex items-center flex-grow py-2 px-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded cursor-pointer transition-colors">
                ...
                <span className="ml-3 text-gray-700 dark:text-gray-300 text-sm font-medium group-hover:text-gray-900 dark:group-hover:text-gray-100 transition-colors">
                  {cat.name}
                </span>
                <span className="ml-auto text-xs text-gray-500 dark:text-gray-400">
                  (
                  {
                    initialProducts.filter((p) =>
                      p.category_group?.some((sc) =>
                        sc._id === cat._id || getAllDescendantIds(cat).includes(sc._id)
                      )
                    ).length
                  }
                  )
